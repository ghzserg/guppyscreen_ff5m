...
SpoolmanPanel::SpoolmanPanel(KWebSocketClient &c, std::mutex &l)
  ...
  , reload_btn(controls, &refresh_img, _("Refresh") /* "Обновить" */, &SpoolmanPanel::_handle_callback, this)
  , back_btn(controls, &back, _("Back") /* "Назад" */, &SpoolmanPanel::_handle_callback, this)
  , active_id(-1)
  , sorted_by(SORTED_BY_ID)
{
  ...
  lv_label_set_text(label, _("Show Archived") /* "Показать архивные" */);
...
void SpoolmanPanel::populate_spools(std::vector<json> &sorted_spools) {
  if (!spools.empty()) {
    ...
    lv_table_set_cell_value(spool_table, 0, 1, _("Name") /* "Имя" */);
    lv_table_set_cell_value(spool_table, 0, 2, "MAT");
    lv_table_set_cell_value(spool_table, 0, 4, _("Remain\nWeight") /* "Осталось\nВес" */);
    lv_table_set_cell_value(spool_table, 0, 5, _("Remain\nLength") /* "Осталось\nДлина" */);
